 ...
 * 
 * 생성자 없음(단독객체 생성 불가)
 * 구현된 메소드는 가질 수 없다(추상메소드만 가능)
 * 필드는 static final 상수만 가능
 * data type의 역할을 할 수 있다
 * interface type의 참조값이 필요하면 implements 클래스를 만들어서 객체를 생성해야함
 * 클래스 상속은 단일 상속이지만 인터페이스는 다중 구현이 가능하다
 */

public interface Remocon {
	//필드도 정의할 수 있지만 static final 만 가능함
